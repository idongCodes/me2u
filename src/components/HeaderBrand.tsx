...
export default function HeaderBrand({ isLoggedIn = false }: HeaderBrandProps) {
  ...
  return (
    <>
      {/* Global Backdrop - positioned outside the header to cover everything */}
      {isMenuOpen && (
        <div 
          className="fixed inset-0 z-[60] bg-black/20 backdrop-blur-[2px] transition-all duration-300"
          onClick={() => setIsMenuOpen(false)}
        />
      )}

      <div className="sticky top-0 z-[70] w-full bg-white/80 backdrop-blur-md border-b border-gray-100/50">
        <div className="flex items-center justify-between px-6 py-4 relative z-[80]">
          <Link 
            href="/" 
        ...
        {/* Dropdown Menu */}
        <div 
          className={`absolute top-full left-0 w-full bg-white/95 backdrop-blur-md border-b border-gray-100/50 shadow-xl flex flex-col overflow-hidden transition-all duration-300 ease-in-out z-[80] ${
